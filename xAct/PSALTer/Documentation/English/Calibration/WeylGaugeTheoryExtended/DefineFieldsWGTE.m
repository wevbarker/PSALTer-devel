...
(*==============================*)
(*  Defining the gauge fields  *)
(*==============================*)
Subsection@"Defining gauge fields and conjugate sources";
Comment@"We first define the gauge fields and conjugate sources used in the analysis.";

Comment@"This is the compensator field:";
DefField[Compensator[],PrintAs->"\[Phi]",PrintSourceAs->"\[Rho]"];
Comment@"This is the Weyl vector:";
DefField[WeylVector[-d],PrintAs->"\[ScriptCapitalB]",PrintSourceAs->"\[ScriptCapitalJ]"];
Comment@"This is the perturbation tetrad:";
DefField[WeylTranslationalGaugeFieldPerturbation[-i,-j],PrintAs->"\[ScriptF]",PrintSourceAs->"\[Tau]"];
Comment@"This is the rotational gauge field:";
DefField[WeylRotationalGaugeField[a,c,-d],Antisymmetric[{a,c}],PrintAs->"\[ScriptCapitalA]",PrintSourceAs->"\[Sigma]"];

(*We also define the coupling constants, following Lin's notation*)
DefConstantSymbol[lLambda,PrintAs->"\[Lambda]"];
DefConstantSymbol[lR1,PrintAs->"\!\(\*SubscriptBox[\(\[ScriptR]\), \(1\)]\)"];
DefConstantSymbol[lR2,PrintAs->"\!\(\*SubscriptBox[\(\[ScriptR]\), \(2\)]\)"];
DefConstantSymbol[lR3,PrintAs->"\!\(\*SubscriptBox[\(\[ScriptR]\), \(3\)]\)"];
DefConstantSymbol[lR4,PrintAs->"\!\(\*SubscriptBox[\(\[ScriptR]\), \(4\)]\)"];
DefConstantSymbol[lR5,PrintAs->"\!\(\*SubscriptBox[\(\[ScriptR]\), \(5\)]\)"];
DefConstantSymbol[lC1,PrintAs->"\!\(\*SubscriptBox[\(\[ScriptC]\), \(1\)]\)"];
DefConstantSymbol[lT1,PrintAs->"\!\(\*SubscriptBox[\(\[ScriptT]\), \(1\)]\)"];
DefConstantSymbol[lT2,PrintAs->"\!\(\*SubscriptBox[\(\[ScriptT]\), \(2\)]\)"];
DefConstantSymbol[lT3,PrintAs->"\!\(\*SubscriptBox[\(\[ScriptT]\), \(3\)]\)"];
DefConstantSymbol[lXi,PrintAs->"\[Xi]"];
DefConstantSymbol[lNu,PrintAs->"\[Nu]"];
(*Here is the Einstein gauge for the compensator field*)
DefConstantSymbol[lPhi0,PrintAs->"\!\(\*SubscriptBox[\(\[Phi]\), \(0\)]\)"];
